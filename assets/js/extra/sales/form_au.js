...
function Cancel()
{
	...
	$.ajax({
		...
		success: function (res) {
			if(res.data.success) {
				//window.location = BASEURL+"sales/salesf";
...
function Submit()
{
	...
	$.post(BASEURL+"sales/salessub/", { method: 'submit_au', id: id.val(), campaign: campaign.html(), type: type.val(), title: title.val(), first: first.val(), middle: middle.val(), last: last.val(), dob: dob.val(), email: email.val(), mobile: mobile.val(), physical: physical.val(), postal: postal.val(), id_type: id_type.val(), id_num: id_num.val(), abn: abn.val(), abn_status: abn_status.html(), position: position.val()},
	function(res) {
		var data = res.data;
		if (data.success){
			sId = data.sId;
			$("#saleId").val(sId);
			$( "#submitedSaleMsg" ).html(data.message);
			var aConfirmationButtons;
			if (data.bCanVerify === 1) {
				aConfirmationButtons = {
					Done: function() {
						window.location= BASEURL+"sales/";
					},
					Verify: function() {
			...
			$('#dialog-form5').dialog('option', 'buttons', aConfirmationButtons);
